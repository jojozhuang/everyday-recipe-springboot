...
public class AuthControllerOld extends BaseController {
  /*
  @Autowired
  ...
  public ResponseEntity<User> signUp(@Valid @RequestBody SignUpDTO signUpDTO) {
    ...
    //user.setRoles(Set.of(Role.STARTER));
    user.setSalt(CryptoUtil.salt());
    user.setHash(CryptoUtil.hash(signUpDTO.getPassword(), user.getSalt()));
